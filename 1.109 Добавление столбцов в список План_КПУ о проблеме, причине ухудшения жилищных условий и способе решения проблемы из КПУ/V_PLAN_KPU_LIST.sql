...
SELECT affair.okrug_id
        ...
        , --
          ...
          -- #25.03.2013Dik Изменил логику подавления "0"  в affair_plan.opd (OPD) для отображения affair_plan.opd как NUMBER
          -- #27.03.2013Dik Изменил логику формирования поля status из V_AFFAIR_PLAN_STATUS,
          --
          --to_char(Affair.stand_year,'0000')||'-'||to_char(Delo_Num,'00000'),
          --substr(get_affair_num_1(Affair.STAND_YEAR,Affair.Delo_Num),1,200),
          -- 24.04.2013 onovikova задача 1.87 Изменено поле sub_rs, добавлено поле sub_date
          --#D14.08.2013 Dik Заведение новых полей - Задача 1.109
...
-- #27.03.2013Dik
         case
             when NVL(affair_plan.status,-1)< 0 then ''
             else
             (select v.name from  V_AFFAIR_PLAN_STATUS v where affair_plan.Status = v.ID)
         end  as status
-- /#27.03.2013Dik
        ,plan_kpu_list.note
        ...
        ,fn_get_subsid(affair_plan.affair_plan_id, affair.affair_id) AS sub_rs  --24.04.2013 onovikova задача 1.87
        ,affair.reason2_num
        ...
        ,FN_get_subsid_date(affair_plan.affair_plan_id, affair.affair_id) AS sub_date, --13.05.2013 onovikova задача 1.87
-- #D14.08.2013
         ...
         PROBLEM_.DATA_D    as DATE_PROBLEM_PERES,
