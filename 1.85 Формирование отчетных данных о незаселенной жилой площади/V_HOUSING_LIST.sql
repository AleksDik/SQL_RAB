...
SELECT                                                          /*- RULE */
          ...
          --  #Dik04032013 Заполнение атрибктов  'КПР', 'КПРм', 'МРк', 'МРкм' ,'КПИ', 'МРи' полей state_reg_date, agreedate
          --  #Dik_09.04.2013(1.85) добавил поля ur_territory,ur_rasporyadytel,ur_direction,ur_period_for_implement
          --
...
-- /#Dik04032013
          ...
          DECODE (get_sbs_fl (apartment.apart_id),
                  ...
                  2, 'ЮЛ',
                  NULL)
             AS SBS_FL,                                    --06.02.2013 ilonis
          -- DECODE (get_sbs_fl (apartment.apart_id), 1, 'да', NULL) AS SBS_FL,
...
--  #Dik_09.04.2013(1.85)
         CASE
             ...
             THEN
                management_realization_data.
                 Get_ur_territory_title (free_space.freespace_id)
             ELSE
                NULL
          END
             AS ur_territory,
          CASE
             ...
             THEN
                management_realization_data.
                 Get_ur_rasporyadytel_title (free_space.freespace_id)
             ELSE
                NULL
          END
             AS ur_rasporyadytel,
          CASE
             ...
             THEN
                management_realization_data.
                 Get_ur_direction_title (free_space.freespace_id)
             ELSE
                NULL
          END
             AS ur_direction,
          CASE
             ...
             THEN
                management_realization_data.
                 Get_period_for_implement_title (free_space.freespace_id)
             ELSE
                NULL
          END
             AS ur_period_for_implement,
-- / #Dik_09.04.2013(1.85)
          '' AS TP_EXISTS
     FROM apartment,
          free_space,
