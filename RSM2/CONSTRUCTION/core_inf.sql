...
where a.registerid=  1620
order by a.id; --EP_PREMISE_Q  Руч. ввод и историч. сисемы: Помещение
...
select a.*, a.rowid from core_register_attribute a
where a.registerid=1600 and a.referenceid
order by a.id; -- EP_BUILDING_Q    Руч. ввод и историч. системы: Здание
...
select a.*, a.rowid from core_register_attribute a
where a.registerid=1601 
order by a.registerid; -- EP_BUILDING_ADDRESS_Q    Руч. ввод и историч. системы: Адрес
...
-------------------- Справочники ----------------------------------
select c.*  from CORE_REFERENCE c 
where c.referenceid  in (11571);
