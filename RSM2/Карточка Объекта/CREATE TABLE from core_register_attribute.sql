...
v_TEST := 0;  --RUN
 ...
 if v_count <> 1 then 
     dbms_output.put_line(TO_CHAR(v_count)||' таблиц в core_register с кодом '||TO_CHAR(c_core_register_id)); 
     return;
 end if;
 ...
 where cr.registerid = c_core_register_id;
  ...
  if v_count<1 then 
      dbms_output.put_line('Создание таблицы  '||c_table_name_o);
 ...
 --return;
  ...
  if v_count<1 then 
      dbms_output.put_line('Создание таблицы '||c_table_name);
      ...
      if v_TEST = 0 then
       ...
       dbms_output.put_line('Создание полей таблицы '||c_table_name);  
  end if;
  ...
  for Rec in (
    ...
    loop
      ...
      if v_count = 0 then
         dbms_output.put_line('Создание поля '||v_field_name||' таблицы '||c_table_name);
        ...
        dbms_output.put_line('Поле '||v_field_name||' уже есть в таблице '||c_table_name); 
      ...
      if  v_field_name is not NULL then
       ...
       if v_count = 0 then
          dbms_output.put_line('Создание поля '||v_field_name||' таблицы '||c_table_name);
           v_type := 'number';
         v_alter_table := REPLACE (REPLACE( c_alter_table, '#TB', c_table_name), '#CL', v_field_name) || v_type || v_nn;   
         v_comment     := REPLACE (REPLACE( c_comment, '#TB', c_table_name), '#CL', v_field_name) ||''''||TRIM(rec.name)||' код''';
          if v_TEST = 0 then
           EXECUTE IMMEDIATE v_alter_table;
           EXECUTE IMMEDIATE v_comment;
          else
