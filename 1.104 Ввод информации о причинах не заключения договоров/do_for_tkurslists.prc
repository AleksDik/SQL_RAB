    ...
    IF l_c = 3 THEN
            ...
            FROM (SELECT 1 AS cmd_id
                  ...
                  SELECT 3 AS cmd_id
                   ...
                   WHERE active_menus.session_id(+) = active_sessions.session_id
                     ...
                     AND NVL (kurs3_var.plan_priv, 0) = 2   --06.02.20013 lonis переделка для SUPER
                     AND ROWNUM = 1
                  UNION ALL
                  SELECT 4 AS cmd_id
     ...
     --ilonis  24.12.2012     передача технического паспорта 
              UNION ALL
                  SELECT 9 AS cmd_id
                        ,    'CreateObject;ClassName=TMenuItem;Owner=Self;Name=miPasportTrafficAdd;Action=aPasportTrafficAdd;'
                          || 'Caption=Ввод данных о движении ТП;ItemsAdd=PopupMenu1;Visible='||DECODE(get_user_priv(113),1,1,0)||';Enabled=1;' AS cmd_lines
                    FROM active_menus, active_sessions
                   WHERE active_menus.session_id(+) = active_sessions.session_id
                     AND active_sessions.session_id = USERENV ('SESSIONID')
                     AND 0 =
          ...
          -- #23.08.2013 Dik
                        ...
                        UNION ALL
                  SELECT 12 AS cmd_id
                        ,    'CreateObject;ClassName=TMenuItem;Owner=Self;Name=miReasonsNSAgr;Action=aReasonsNSAgr;'
                          || 'Caption=Причины не заключения договоров;ItemsAdd=PopupMenu1;Visible='||DECODE(get_user_priv(109),1,1,0)||';Enabled=1;' AS cmd_lines
                    FROM active_menus, active_sessions
                   WHERE active_menus.session_id(+) = active_sessions.session_id
                     AND active_sessions.session_id = USERENV ('SESSIONID')
                     AND 0 =
                           (SELECT COUNT (active_menus.menu_handle)
                             ...
                             WHERE active_menus.session_id = USERENV ('SESSIONID')
                               AND active_menus.parent_id = do_for_tkurslists.form_handle
                               AND active_menus.menu_text = 'miReasonsNSAgr')  
                            --  AND kurs3_var.plan_priv = 1   
                   -- AND ( ( select get_user_priv(109) from dual )>0) --привилегия
                     AND ROWNUM = 1        
          ...
          -- / #23.08.2013 Dik                       
                  ...
                  UNION ALL
                  SELECT 10 AS cmd_id, 'Window;Handle=' || form_handle || ';SearchListDisp=1;' AS cmd_lines
                    FROM DUAL
                   WHERE l_c = 3 AND l_n = 1 AND kurs3_var.product_name = 'ARMRealiz'
				  union all 
				  SELECT 11 AS cmd_id, 'Window;Handle=' || form_handle || ';Caption=' || l_name || ': ' || l_name1 || ';' AS cmd_lines
				    from dual
