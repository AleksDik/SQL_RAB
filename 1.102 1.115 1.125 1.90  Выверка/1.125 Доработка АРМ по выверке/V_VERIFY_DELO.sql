...
SELECT                                                                   --
          ...
          --  19.11.2013  DIK Добавление поля  - is_affair_change_after (задача 1.115)
          --  11.04.2014  DIK Добавление статуса 5, 'УДАЛЕНО', (задача 1.125)
          --
          ed.delo_id,
          aff.affair_id,
          aff.affair_stage,
          ...
          || TRIM (
                TO_CHAR (
                   DECODE (aff.sq_type,
                           1, aff.kk_num,
                           get_room_cnt (aff.affair_id, aff.affair_stage))))
          || ' комн. в '
          || LOWER (
                kursiv.padeg.
                 get_fio_fromstr (get_classifier_h (5, aff.sq_type), 'ж', 3))
             AS kpu_num,
...
-- / 25.06.2013
          ...
          DECODE (NVL (ed.status, 0),
                  ...
                  4, 'ВОЗВРАЩЕНО',
                  1, 'ОТСКАНИРОВАНО',
                  5, 'УДАЛЕНО', ----  11.04.2014  DIK
                  '')
             AS status_label,
          edda.document_id AS scan_doc_id,
          get_affair_person_cnt (aff.affair_id, aff.affair_stage) AS affair_person_cnt
--  19.11.2013 DIK
        ,case when pkg_scan.get_is_affair_change_after (ed.delo_id) = 1 then 'Изменено ДЖП' else '' end as is_affair_change_after
--/ 19.11.2013 DIK
     FROM affair aff,
          scan.ea_delo ed,
