CREATE OR REPLACE PROCEDURE link_kurs_with_scan AS
  ...
  -- 23.10.2013   Ikonnikov  Доработана процедура (стр. 287) удаления задвоенных документов, теперь только имеющие row_status =0 (Архивные)
...
--  05.02.2014 Dik#    попытка избежать привязки более одного паспорта по человеку
...
--             VALUES (recp.document_id, recp.document_version, recp.object_type_id, recp.object_rel_id, recp.last_change, recp.row_status);
 --  05.02.2014 Dik# 
        select count(*) into c FROM scan.ea_document_attr att 
        WHERE (((att.object_type_id = 7) AND (att.object_rel_id = recp.object_rel_id) ) OR (att.document_id = rec.document_id)) and att.row_status=1;
       ...
       if c = 0 then
        INSERT INTO scan.ea_document_attr (document_id, document_version, object_type_id, object_rel_id, last_change, row_status,user_id)
             VALUES (recp.document_id, cnt, recp.object_type_id, recp.object_rel_id, recp.last_change, recp.row_status,-100); -- -100 автомат
        end if;      
 --/  05.02.2014 Dik# 
       END IF;
...
--             VALUES (recp.document_id, recp.document_version, recp.object_type_id, recp.object_rel_id, recp.last_change, recp.row_status);
  ...
  -- 23.10.2013 Ikonnikov  Доработана процедура удаления задвоенных документов, теперь только имеющие row_status =0 (Архивные)
  DELETE FROM scan.ea_document_attr
        WHERE     object_type_id = 7
              ...
              AND document_id <> (SELECT MAX (document_id) m_id
                                   FROM scan.ea_document_attr edoa
                                  WHERE edoa.object_type_id = 7 
                                    AND edoa.row_status=1
                                    AND edoa.object_rel_id = ea_document_attr.object_rel_id)
              AND row_status =0;
