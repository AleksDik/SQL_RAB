CREATE OR REPLACE PROCEDURE kursiv.on_form_command_kurs3 (form_class_name IN     VARCHAR2
  ...
  --  15.03.2013  dikan печать техпаспорта  
  --  20.03.2012  AVB         Добавил обработку do_for_frmDocPoor
  -- 25.09.2012 Ilonis -добавил обработку  DO_FOR_TFRMUFRS     
  -- 21.02.2013 Ilonis -добавил обработку  DO_FOR_TECHPASPORTACTPRINT
  -- 03.06.2013 onovikova     Добавила обработку DO_FOR_TFrmActPP
  -- 28.11.2013 Dik           Добавил обработку форм ('FRMVYVREPRESULTVC','FRMVYVREPUSERSVC') ввода параметров для отчетов:
...
BEGIN
  ...
  -----------------------------------------------------------------------------------------------------------  
  ELSIF UPPER (form_name) = 'FRMOPDEDIT' THEN 
    do_for_frmOPDEdit (formclassname, form_name, form_handle, form_event, out_props, qcommand);
  --------------------------------------------------------------------------------------------------------------
  ...
  -----------------------------------------------------------------------------------------------------------
  -- 28.11.2013 Dik
  ELSIF UPPER (form_name) IN ('FRMVYVREPRESULT','FRMVYVREPRESULTVC', 'FRMVYVREPUSERS','FRMVYVREPUSERSVC') AND (UPPER (kurs3_var.product_name) = 'KURS3') THEN
  ...
  -----------------------------------------------------------------------------------------------------------
   ELSIF UPPER (form_class_name) = 'TFRMUFRS' THEN     --25.09.2012 Ilonis  
   DO_FOR_TFRMUFRS (formclassname, form_name, form_handle, form_event, out_props, qcommand);
  -----------------------------------------------------------------------------------------------------------
   ELSIF UPPER (form_class_name) = 'TFRMACTPP' THEN    --03.06.2013 onovikova
   DO_FOR_TFrmActPP (formclassname, form_name, form_handle, form_event, out_props, qcommand);
  -----------------------------------------------------------------------------------------------------------         
ELSIF UPPER (form_class_name) = 'TFRM_TECHPASPORTACTPRINT' THEN    --печать техпаспорта  22.02.2013 ilonis
     DO_FOR_TECHPASPORTACTPRINT (formclassname, form_name, form_handle, form_event, out_props, qcommand);  
--ELSIF UPPER (form_class_name) = 'TTPREPORTDIALOG' THEN    --печать техпаспорта  15.03.2013 dikan
 --    DO_FOR_TECHPASPORTREPORTSPRINT (formclassname, form_name, form_handle, form_event, out_props, qcommand);  
