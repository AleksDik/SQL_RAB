...
--  21.11.2013 Dik#   Добавлены новые процедуры для использоания в интерфейсе
...
CREATE OR REPLACE PACKAGE BODY pkg_scan AS
  ...
  PROCEDURE setstatus (p_delo_id_ NUMBER, p_status_ NUMBER, p_comment_ VARCHAR2, p_msg_ IN OUT VARCHAR2) AS
    tmp_ VARCHAR2 (200);
  BEGIN
    IF NOT (NVL (p_status_, 0) IN (1, 2, 3, 4)) THEN
...
-- 29.10.2013 Dik добавил  изменение last_change в  scan.ea_delo 
 ...
 WHERE delo_id = p_delo_id;
  ...
  BEGIN
    OPEN all_room FOR
      SELECT room.building_id, room.apart_id, room.room_num, room.characteristic, room.room_space,room.npp_bti, affair.affair_id, affair.affair_stage, 1 indelo
      ...
      UNION
      SELECT room.building_id, room.apart_id, room.room_num, room.characteristic, room.room_space, room.npp_bti, affair.affair_id, affair.affair_stage, 0 indelo
        FROM affair, room
